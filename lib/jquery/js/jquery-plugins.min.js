...
1)})}}function keypressEvent(e,keyCode){e=e||window.event;var key=keyCode||e.which||e.charCode||e.keyCode,decimalKeyCode=settings.decimal.charCodeAt(0);if(key===undefined)return false;if(key===229)return false;if ($input.prop('readonly') || $input.prop('disabled')) {
    return false;
}if((key<48||key>57)&&(key!==decimalKeyCode||!settings.reverse))if(key==45&&$input.val()==""){handleAllKeysExceptNumericalDigits(key,e);applyMask(e);return false}else return handleAllKeysExceptNumericalDigits(key,e);else if(!canInputMoreNumbers())return false;else{if(key===decimalKeyCode&&shouldPreventDecimalKey())return false;
