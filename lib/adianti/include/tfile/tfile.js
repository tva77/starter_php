function FileUploader(input_id, parent_container, service_action, complete_action, error_action, file_handling, image_gallery, popover, limitSize)
{
    ...
    this.showMessage = function(type, message)
    {
        ...
        if(this.drop_zone)
        {
            if (type == 'success') 
            {
                const upload_success = _t('file_uploaded_successfully');
                $(`#tfile_dropzone_info_${this.input_id}`).html(`
                    <div class="alert alert-success" role="alert">${upload_success}</div>    
    ...
    this.enableDropZone = function()
    {
        this.drop_zone = true;
        const dropZone = $(`#tfile_dropzone_${this.input_id}`);
        const hiddenFileInput = $(`#${this.input_id}`);
        ...
        hiddenFileInput.on('change', function () {
            const fileName = hiddenFileInput[0].files[0]?.name;
            
        });

        $(document).on('click', '.tfile_del_icon', function () {
...
function tfile_start( input_id, parent_container, service_action, complete_action, error_action, file_handling, image_gallery, popover, limit_size, drop_zone)
{
    $(function() {
        var file = new FileUploader(input_id, parent_container, service_action, complete_action, error_action, file_handling, image_gallery, popover, limit_size, drop_zone);
