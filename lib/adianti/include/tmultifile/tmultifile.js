...
function MultiFileUploader(input_id, obj_file, service_action, parent_container, complete_action, file_handling, image_gallery, popover, limitSize)
{
    ...
    this.createStatusRow = function()
    {
        ...
        $(file_del_icon).click(
            function(){
                var file_data = that.getData(file_input_hidden);
                ...
                if (file_data.delFile == file_data.newFile) {
                    file_data = '';
                    that.setData(file_data, file_input_hidden);
        ...
        if (file_name) {
            ...
            if ($.inArray( file_name.split('.').pop().toLowerCase(), [ "png", "jpg", "jpeg", "gif", "svg", "webp", "ico", "fav" ] ) > -1) {
                ...
                if (that.image_gallery.enabled == '1') {
                    ...
                    // Cria o ícone baseado no tipo de arquivo
                    var iconClass = this.getFileIcon(file_name);
                    var fileIcon = $('<i />', { 
                        'class': iconClass,
                        'style': 'font-size: ' + Math.min(that.image_gallery.height, that.image_gallery.width) + 'px; color: #666; display: flex; align-items: center; justify-content: center; height: ' + that.image_gallery.height + 'px; width: ' + that.image_gallery.width + 'px;'
                    });
                    
                    file_link.append(fileIcon);
                    
                    if (that.popover.enabled == '1') {
                        // Conteúdo do popover para arquivos não-imagem
                        var pop_content_file = '<div style="text-align: center; padding: 20px;"><i class="' + iconClass + '" style="font-size: 48px; color: #666; margin-bottom: 10px;"></i><br><strong>' + file_name + '</strong></div>';
                        var pop_template = '<div class="popover" role="tooltip" style="z-index:100000;max-width:800px"><div class="arrow"></div><h3 class="popover-title popover-header"></h3><div class="popover-content popover-body"><div class="data-content"></div></div></div>';
                        
                        file_link.popover({
                            trigger: "hover",
                            'content': pop_content_file,
                            'html': true,
                            'container': 'body',
                            'template': pop_template,
                            title: that.popover.title || "Arquivo"
                        });
                    }
                }
                else
    ...
    this.getFileIcon = function(fileName) {
        ...
        // Tipos de arquivo e seus ícones correspondentes no Font Awesome 5
        const iconMap = {
            // Documentos
            'pdf': 'fas fa-file-pdf',
        ...
        return iconMap[extension] || 'fas fa-file'; // Ícone padrão se não encontrar
    }
    
    // Get file data
    this.getData = function(file_input_hidden)
    {
        if(typeof file_input_hidden == 'undefined')
        {
            var file_data = decodeURIComponent(that.file_input_hidden.val());
        }
        else{
            var file_data = decodeURIComponent(file_input_hidden.val());
        }
    ...
    // Set file data
    this.setData = function(data, file_input_hidden)
    {
        if(typeof file_input_hidden == 'undefined')
        {
            file_input_hidden = that.file_input_hidden;
        }
        if (data) {
            $(file_input_hidden).val(encodeURIComponent(JSON.stringify(data)));
        }
        else {
    ...
    this.setValue = function(values)
    {
        ...
        for (var index in values) 
        {
            ...
            {
                const row = this.createStatusRow();

                if(typeof data.delFile != 'undefined')
                {
                    row.hide();
                }
