    ...
    $.each(fields, function(index, field)
    {
        ...
        if (typeof field_id !== "undefined")
        {
            ...
            else if (field_component =='tfile')
            {
                ...
                if(parent.find('.tfile_dropzone').length > 0) {
                    parent.find('.tfile_dropzone').attr('id', 'tfile_dropzone_' + new_id);

                    parent.find('.tfile_dropzone_info').attr('id', 'tfile_dropzone_info_' + new_id);

                    parent.find('.tfile_dropzone_info').empty();
                }
...
function tfieldlist_column_count(field_name)
{
    var count = 0;
    $('[name="'+field_name+'[]"]').each(function (k,v) {
        var total_raw = tentry_get_data_by_id( $(v).attr('id') );
...
function tfieldlist_update_sum(fieldlist_name, field_name, callback)
{
    setTimeout( function() {
        ...
        var maskParts = [];
        var form = $('table[name='+fieldlist_name+']').closest('form');
        
        if (mask) {
            maskParts = mask.split('');
        }
        const grandTotal = $('[name=grandtotal_'+field_name+']');
        const totalFormated = number_format( total, maskParts[0]??2, maskParts[1]??',', maskParts[2]??'.');
        
        grandTotal.val( totalFormated );
        
        if (grandTotal.data('total-form-field')) {
            const totalFormField = grandTotal.data('total-form-field');

            form.find('[name="'+totalFormField+'"]').val(totalFormated)
        }
...
function tfieldlist_update_count(fieldlist_name, field_name, callback)
{
    setTimeout( function() {
        var total = tfieldlist_column_count(field_name);
        var mask  = $('[name^='+field_name+']:first').data('nmask');
        var maskParts = [];
        ...
        const grandTotal = $('[name=grandtotal_'+field_name+']');
        grandTotal.val( number_format( total, maskParts[0]??2, maskParts[1]??',', maskParts[2]??'.') );
