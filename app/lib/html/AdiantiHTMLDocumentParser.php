...
class AdiantiHTMLDocumentParser
{
    ...
    private $DOMOptions;
    private $repeatData = [];
    private $pageScriptFunction = null;
                    ...
                    if (isset($this->details[$model]))
                    {
                        ...
                        if ($objects)
                        {
                            ...
                            foreach ($objects as $object)
                            {
                                ...
                                foreach ($attributes as $attribute)
                                {
                                    $new_row = str_replace('{{'.$attribute.'}}',(string) $object->$attribute, $new_row);
                                    $new_row = str_replace('{$'.$attribute.'}', (string) $object->$attribute, $new_row);
    ...
    public function setPageScriptFunction($function)
    {
        $this->pageScriptFunction = $function;
    }
    ...
    public function saveAsPDF($filename, $format = 'A4', $orientation = 'portrait')
    {
        ...
        if($this->pageScriptFunction)
        {
            $canvas = $dompdf->getCanvas();
            $canvas->page_script($this->pageScriptFunction);
        }
